function loadJSON() {
    fetch("https://www.googleapis.com/books/v1/users/109475590769927821044/bookshelves/1004/volumes?q=php&maxResults=40")
...
function presentBooksAsCards(jsondata) {
    row = document.querySelector("#deckare");
    row.innerHTML = '';
...
/* var allDeckare = bookscussler.concat(bookstopp);

let divcontainer4 = document.querySelector('#deckare');

for (let i = 0; i < allDeckare.length; i++) {
    let divCol=document.createElement("div");
    divCol.classList.add("col-lg-2");
    divCol.classList.add("col-md-3");
    divCol.classList.add("col-4", "mb-2");           
    ...
    let h6=document.createElement("h6");
    h6.classList.add("card-title", "my-1");
    let textH6=document.createTextNode(allDeckare[i].title);
    h6.insertAdjacentText("beforeend",textH6.textContent);
    divCardBody.insertAdjacentElement("beforeend",h6);
    
    let p=document.createElement("p");
    p.classList.add("card-text", "mb-2");
    pText=document.createTextNode(allDeckare[i].author);
    p.insertAdjacentText("beforeend",pText.textContent);
    divCardBody.insertAdjacentElement("beforeend",p);

    let h5=document.createElement("h5");
    h5.classList.add("text-danger", "mt-auto");
    let h5Text=document.createTextNode(allDeckare[i].price+" kr");
    h5.insertAdjacentText("beforeend",h5Text.textContent);
    divCardBody.insertAdjacentElement("beforeend",h5);
    ...
    a.insertAdjacentElement("beforeend",aIcon);
    a.addEventListener("click", function(){
        window.location.href="merinfo.html?title="+allDeckare[i].title;
    });
    ...
    divcontainer4.insertAdjacentElement("beforeend",divCol);
